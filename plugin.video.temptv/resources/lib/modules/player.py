...
class player(xbmc.Player):
    ...
    def getMeta(self, meta):
        ...
        try:
            ...
            meta = [i for i in meta if self.year == str(i['year']) and (t == cleantitle.get(i['title'])
                                                                        or t == cleantitle.get(i['originaltitle']))][0]
    ...
    def libForPlayback(self):
        try:
            ...
            if self.content == 'movie':
                rpc = '{"jsonrpc": "2.0", "method": "VideoLibrary.SetMovieDetails", ' \
                      '"params": {"movieid" : %s, "playcount" : 1 }, "id": 1 }' % str(self.DBID)
            elif self.content == 'episode':
                rpc = '{"jsonrpc": "2.0", "method": "VideoLibrary.SetEpisodeDetails", ' \
                      '"params": {"episodeid" : %s, "playcount" : 1 }, "id": 1 }' % str(self.DBID)
    ...
    def get(self, name, imdb, season, episode):
        try:
            ...
            langDict = {
                ...
                'Portuguese': 'por,pob', 'Portuguese(Brazil)': 'pob,por', 'Romanian': 'rum',
            ...
            codePageDict = {
                'ara': 'cp1256', 'ar': 'cp1256', 'ell': 'cp1253', 'el': 'cp1253',
                'heb': 'cp1255', 'he': 'cp1255', 'tur': 'cp1254', 'tr': 'cp1254',
                'rus': 'cp1251', 'ru': 'cp1251'}
            ...
            if not (season is None or episode is None):
                result = server.SearchSubtitles(token, [{'sublanguageid': sublanguageid, 'imdbid': imdbid,
                                                         'season': season, 'episode': episode}])['data']
                fmt = ['hdtv']
            else:
...
class bookmarks:
    ...
    def reset(self, currentTime, totalTime, name, year='0'):
        try:
            ...
            dbcon = database.connect(control.bookmarksFile)
            dbcur = dbcon.cursor()
            dbcur.execute("CREATE TABLE IF NOT EXISTS bookmark (""idFile TEXT, "
